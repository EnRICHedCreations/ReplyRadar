...
export function validateQuery(query: string) {
  ...
  if ((query.match(/"/g) || []).length % 2)
    throw new Error("Close the quotation marks in your query.");
  let depth = 0;
...
export const radarSchema = z.object({
  ...
  query: z.string().transform((v, ctx) => {
    try {
  ...
  scan_interval_seconds: z.number().int().min(30).max(86400),
  ...
  active_schedule_json: scheduleSchema.default(null),
  minimum_score: z.number().int().min(0).max(100).default(70),
  integration_ids: z.array(z.uuid()).max(20).default([]),
});
...
export function scorePost(p: SocialPost, now = new Date()) {
  ...
  const intent = /looking for|need a|budget|buy|pay for/.test(t)
    ? "BUYING_INTENT"
    : /recommend|alternative|what do you use/.test(t)
      ? "RECOMMENDATION_REQUEST"
      : /expensive|frustrat|broken|hate|bill/.test(t)
        ? "COMPLAINT"
        : t.includes("?")
          ? "QUESTION"
          : /versus|competitor/.test(t)
            ? "COMPETITOR_MENTION"
            : /@\w+/.test(t)
              ? "BRAND_MENTION"
              : "GENERAL_DISCUSSION";
...
export function suggestQuery(description: string, goal = "customers") {
  ...
  const topic = words.slice(0, 3).join(" ") || "deployment";
  if (/vercel/i.test(description))
    return "vercel (expensive OR pricing OR bill OR alternative) -is:retweet";
  return goal === "brand"
    ? `"${topic}" -is:retweet`
    : `("looking for" OR recommend OR alternative) "${topic}" -is:retweet`;
}
