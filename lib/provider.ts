...
export class MockProvider implements SocialSearchProvider {
  async search({ query, cursor, now = new Date() }: SearchInput) {
    ...
    if (query.includes("[rate-limit]"))
      throw new ProviderError("MOCK_RATE_LIMIT", true, 120);
...
export class XProvider implements SocialSearchProvider {
  async search({ query, cursor }: SearchInput) {
    ...
    if (!process.env.X_BEARER_TOKEN)
      throw new ProviderError("X_CREDENTIALS_MISSING", false);
    let token: string | undefined;
    let newest = cursor || undefined;
    ...
    for (let page = 0; page < 10; page++) {
      ...
      if (token) u.searchParams.set("next_token", token);
      ...
      if (!res.ok)
        throw new ProviderError(
          ...
          Math.max(
            60,
            Number(res.headers.get("x-rate-limit-reset") || 0) -
              Math.floor(Date.now() / 1000),
          ),
        );
      const body = await res.json();
      if (body.errors?.length)
        throw new ProviderError("X_PARTIAL_RESPONSE", true);
      posts.push(
        ...(body.data || []).map((p: any) =>
          normalizeX(p, body.includes?.users || []),
        ),
      );
      if (page === 0) newest = body.meta?.newest_id || newest;
      token = body.meta?.next_token;
      if (!token) return { posts, cursor: newest };
    } // Do not advance past unprocessed pages; force a narrower query.
