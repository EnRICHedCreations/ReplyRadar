...
export async function scanNext(search?: SocialSearchProvider) {
  return transaction(async (c) => {
    ...
    if (r.last_scan_at && +new Date(r.last_scan_at) + interval * 1000 > Date.now()) return defer("WAITING", Math.ceil((+new Date(r.last_scan_at) + interval * 1000 - Date.now()) / 1000));
    ...
    try {
      ...
      await c.query("update scan_runs set status='failed',completed_at=now(),duration_ms=$2,error_code=$3,error_message=$4,retryable=$5 where id=$1", [run, Date.now()-start, e.code, detail, e.retryable]);
      await c.query("update radars set last_scan_at=now(),last_error='PROVIDER_DELAYED',next_scan_at=now()+make_interval(secs=>$2) where id=$1", [r.id, Math.max(interval, e.retryAfter, e.retryable ? 0 : 3600)]);
      await c.query("delete from scan_jobs where radar_id=$1", [r.id]);
