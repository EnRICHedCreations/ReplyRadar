...
export default function RadarEditor({id}:{id?:string}){
 ...
 const [maxResults,setMaxResults]=useState(25),[busy,setBusy]=useState(false),[msg,setMsg]=useState(""),[showHelp,setShowHelp]=useState(false),[confirmDelete,setConfirmDelete]=useState(false);
 ...
 async function removeRadar(){if(!id)return;setBusy(true);setMsg("");try{await api("radars/"+id,"DELETE");router.push("/overview");router.refresh()}catch(e){setMsg((e as Error).message);setBusy(false)}}
 ...
 <div className="grid2"><div className="field"><label>How often should we check?</label><select value={form.scan_interval_seconds} onChange={e=>set("scan_interval_seconds",Number(e.target.value))}>{[600,1800,3600,21600,43200,86400].map(n=><option key={n} value={n}>{n<3600?`Every ${n/60} minutes`:n<86400?`Every ${n/3600} hours`:"Every 24 hours"}</option>)}</select></div><div className="field"><label>Posts checked per scan</label><select value={maxResults} onChange={e=>setMaxResults(Number(e.target.value))}>{[10,25,50,75,100].map(n=><option key={n} value={n}>{n} posts · ${(n/100).toLocaleString("en-US",{style:"currency",currency:"USD"})} per scan</option>)}</select><small className="hint">This is a fixed scan price. The selected amount is charged for each successful X scan even when X returns fewer posts or no matching posts.</small></div></div>
 ...
 <div className="notice">Cost protection: each successful scan makes at most one X request and charges the fixed price shown above. A scan will not start unless your credit balance can cover that full amount.</div>
 <div className="row between wrap" style={{gap:12}}><button className="primary" disabled={busy}>{busy?"Saving…":id?"Save radar":"Activate radar"}</button>{id&&(!confirmDelete?<button type="button" className="quiet" disabled={busy} onClick={()=>setConfirmDelete(true)}>Delete radar</button>:<div className="row wrap" style={{gap:8}}><span className="muted">Delete this radar and its history?</span><button type="button" className="quiet" disabled={busy} onClick={()=>setConfirmDelete(false)}>Cancel</button><button type="button" className="quiet" disabled={busy} onClick={removeRadar}>{busy?"Deleting…":"Yes, delete radar"}</button></div>)}</div>
 </form></>
}
