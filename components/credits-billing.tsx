...
export default function CreditsBilling(){
  ...
  return <>
    ...
    {data&&<>
      <div className="stats">
        <div className="panel stat"><span className="eyebrow">AVAILABLE BALANCE</span><strong>{money(data.account.balance_cents)}</strong><small>Monitoring pauses automatically at $0.</small></div>
        <div className="panel stat"><span className="eyebrow">SPENT THIS MONTH</span><strong>{money(data.month_spent_cents)}</strong><small>Usage debited after successful X scans.</small></div>
      ...
      <section className="panel" style={{marginTop:24}}><h2>Per-radar cost controls</h2><p className="muted">Each scan performs a single X Recent Search request. Lower result caps reduce worst-case spend. Scans are currently limited to no more than once every 10 minutes.</p>{data.radars.length?data.radars.map((r:any)=><div className="breakdown-row" key={r.id}><div><strong>{r.name}</strong><div className="muted">{r.enabled?"Active":"Paused"} · max {r.max_results_per_scan} posts / scan</div></div><select aria-label={`Maximum results for ${r.name}`} value={r.max_results_per_scan} disabled={saving===r.id} onChange={e=>budget(r.id,Number(e.target.value))}>{[10,25,50,75,100].map(n=><option value={n} key={n}>{n} posts</option>)}</select></div>):<p className="muted">Create a radar to set its scan budget.</p>}</section>
